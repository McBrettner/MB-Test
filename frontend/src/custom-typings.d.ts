...
declare module 'assets/js/watson-speech' {
	interface Options {
		token: string;
	}
	interface OptionsS2T extends Options {
		autoPlay?: boolean;
		keepMic?: boolean;
		continuous: boolean;
	}
	interface OptionsT2S extends Options {
		text: string;
		voice?: any;
	}
	interface ITextToSpeech {
		synthesize(options: OptionsT2S): void;
		getVoices(options: Options): any;
	}
	export var TextToSpeech: ITextToSpeech;

	interface ISpeechToText {
		recognizeMicrophone(options: OptionsS2T): any;
	}
	export var SpeechToText: ISpeechToText;
}
